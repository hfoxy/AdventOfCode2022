    ...
    public Object run() {
        ...
        for (String line : lines) {
            if (line.startsWith("$")) {
                ...
                LOGGER.debug("Adding {} to {} ({}/{})", name, directory, directory, name);
            }
        }

        int diskSize = 70000000;
        int requiredSize = diskSize - 30000000;
        int totalSize = items.get("").getTotalSize();
        int minDelete = totalSize - requiredSize;

        LOGGER.info("Disk Size     : {}", diskSize);
        LOGGER.info("Required Size : {}", requiredSize);
        LOGGER.info("Total Size    : {}", totalSize);
        LOGGER.info("Minimum Delete: {}", minDelete);

        return items.values().stream().mapToInt(DirectoryItem::getTotalSize).sorted().filter(i -> i >= minDelete).findFirst().orElseThrow();
    }
